...
> **重要说明**：PeMS08 的 170 个传感器全部位于加州高速公路，平均速度范围在 87-114 km/h，没有长期低速（<60 km/h）的传感器。因此不能按"平均速度高低"来分类，而应**按速度标准差（波动程度）来区分传感器类型**。标准差大说明该传感器既有自由流时段也有拥堵时段，基本图散点分布更完整。

选择方法：遍历所有 170 个传感器，计算每个传感器 speed（转换为 km/h 后）的均值和标准差，按标准差降序排列。

- **主传感器（混合状态最明显）**：选择标准差最大的传感器。推荐 sensor 22（mean=87.0 km/h, std=33.1, 拥堵时段占比30.5%），该传感器的基本图散点覆盖从自由流到拥堵的完整范围，最适合做主实验。
- **补充传感器 A（稳定自由流）**：选择标准差最小且均速最高的传感器。推荐 sensor 120（mean=110.5 km/h, std=1.8, 拥堵占比0%），几乎全部时段都是自由流，用来测试 LLM 在单一状态下的表现。
- **补充传感器 B（次混合状态）**：选择标准差排名第二或第三的传感器。推荐 sensor 9（mean=96.1 km/h, std=24.9, 拥堵占比17.1%），用于验证主实验结论的鲁棒性。

> 如果你想通过代码自动选择而非硬编码 sensor ID，逻辑是：按 speed 标准差降序排列，主传感器取第1名，补充B取第3名；按标准差升序排列且均速>105 km/h的传感器中取第1名作为补充A。
